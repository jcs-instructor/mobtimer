...
// These tests use real time (instead of the jest fake timer in other tests), so they are slower but 
// needed to exercise what happens when real time passes, including expiration (which uses an injected 
// function).
describe("Client WebSocket Server Integration", () => {
  ...
  beforeAll(async () => {
    _server = await backendUtils.startMobServer(_port);
  ...
  test.skip("Handle bad message and subsequent request succeeds", async () => {
    _client1.webSocket?.sendToServer(
