...
describe("Client WebSocket Server Integration", () => {
  ...
  test("Create 2 mobs", async () => {
    ...
    const _mobName2 = await joinMobWithAutogeneratedName(_client2, "1");
  ...
  test("Second client joins shared mob in paused state", async () => {
    const mobNameForBothTeams = await joinMobWithAutogeneratedName(_client1, "super-team");
  ...
  test("Handle bad message and subsequent request succeeds", async () => {
    ...
    await joinMobWithAutogeneratedName(anotherClient);
    await anotherClient.waitForLastResponse();
...
async function joinMobWithAutogeneratedName(client: MobSocketTestClient, suffix = "") {
  mobCounter++;
