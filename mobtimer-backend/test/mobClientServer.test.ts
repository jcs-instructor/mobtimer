...
describe("Client WebSocket Server Integration", () => {
  ...
  test("Create mob with alternative websocket", async () => {    
    const anotherClient = await openSocketAlternative(url);
    const mobName = await joinMobWithAutogeneratedName(anotherClient);
    await anotherClient.joinMob(mobName);
  ...
  test("Create mob", async () => {
    const mobName = await joinMobWithAutogeneratedName(_client1);
  ...
  test("Create 2 mobs", async () => {
    const mobName1 = await joinMobWithAutogeneratedName(_client1);
    const mobName2 = await joinMobWithAutogeneratedName(_client2);
  ...
  test("Modify one of two mob timers", async () => {
    ...
    await joinMobWithAutogeneratedName(_client2);
